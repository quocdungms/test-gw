...
def decode_raw_data(data):
    ...
    try:
        ...
        if data_type == 2:  # Type 2: Position and Distances
            x, y, z, quality_factor = struct.unpack('<iiiB', data[1:14])
            # quality_factor = data[13]
            decoded["position"] = {"x": x/1000, "y": y/1000, "z": z/1000}
            ...
            if len(data) <= 13:
                ...
                return None
            x, y, z, quality_factor = struct.unpack('<iiiB', data[1:14])  # Read 3 float values
            # quality_factor = struct.unpack('<B', data[14])  # 1 byte for quality factor
